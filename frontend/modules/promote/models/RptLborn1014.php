...
class RptLborn1014 extends Model {
    ...
    public function search($params = null) {
        ...
        $sql = " select  'null'";
        if (!empty($params['RptLborn1014']['cup'])) {
            $cup = $params['RptLborn1014']['cup'];
...
FROM t_labor
WHERE TIMESTAMPDIFF(YEAR,birth,BDATE) BETWEEN 10 and 14 AND BDATE BETWEEN $start_d AND $end_d	AND LBORN >=1
GROUP BY cid
) l ON l.cid=p.CID
LEFT JOIN chospital_amp h ON h.hoscode = p.HOSPCODE
WHERE	p.age_y BETWEEN 10 AND 14 AND p.sex IN(2)
